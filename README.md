...
| `TWITTER_MEDIA_HOSTS` | Twitterのメディア取得を許可するホストのカンマ区切りリスト（デフォルト: `pbs.twimg.com,video.twimg.com`）※SSRF対策用 |
...
| `TRACKER_DB_PATH` | CrossPostTrackerのsqlite DBファイルパス（デフォルト: `data/tracker.sqlite`） |
| `TRACKER_RETENTION` | Trackerレコードの保持期間（デフォルト: `2160h`、0以下で無期限） |
