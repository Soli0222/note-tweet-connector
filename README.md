...
- Misskey APIトークン
- Twitter APIキー、OAuth 1.0aアクセストークン、OAuth 2.0 Bearer Token、OAuth 2.0 refresh token
...
| `-twitter-user-access-token` | なし | Media API v2 chunked uploadに使うOAuth 2.0 User Access Token。refresh token設定時は任意 |
...
Webhookの作成と一覧取得はOAuth 2.0 Application-Only Bearer Tokenで行います。Developer PortalのAppからBearer Tokenを取得し、ここでは`TWITTER_BEARER_TOKEN`として扱います。`TWITTER_USER_ACCESS_TOKEN`はMedia API v2などのOAuth 2.0 User Context用であり、`GET /2/webhooks`や`POST /2/webhooks`には使いません。
