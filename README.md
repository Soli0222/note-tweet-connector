...
- Misskey APIトークン
- Twitter APIキー、OAuth 1.0aアクセストークン、OAuth 2.0 refresh token
...
note-tweet-connector \
  ...
  -twitter-user-refresh-token="${TWITTER_USER_REFRESH_TOKEN}" \
  -twitter-oauth2-client-id="${TWITTER_OAUTH2_CLIENT_ID}" \
  -twitter-oauth2-client-secret="${TWITTER_OAUTH2_CLIENT_SECRET}" \
...
| `-twitter-user-access-token` | なし | Media API v2 chunked uploadとWebhook / Account Activity APIの操作に使うOAuth 2.0 User Access Token。refresh token設定時は任意 |
| `-twitter-user-refresh-token` | なし | OAuth 2.0 User Access Tokenを更新するためのrefresh token |
| `-twitter-oauth2-client-id` | なし | OAuth 2.0 refresh token flowに使うClient ID |
| `-twitter-oauth2-client-secret` | なし | OAuth 2.0 refresh token flowに使うClient Secret。public clientでは未指定可 |
...
Media API v2 chunked uploadやWebhook / Account Activity APIの操作にはOAuth 2.0 User Access Tokenが必要です。通常運用では`-twitter-user-refresh-token`と`-twitter-oauth2-client-id`を設定してください。`-twitter-user-access-token`は任意で、未指定の場合は初回利用時にrefresh tokenからaccess tokenを取得します。refresh tokenを取得する認可では`offline.access` scopeが必要です。
...
| `TWITTER_USER_ACCESS_TOKEN` | いいえ | Twitter OAuth 2.0 User Access Token。refresh token設定時は任意 |
| `TWITTER_USER_REFRESH_TOKEN` | はい | Twitter OAuth 2.0 refresh token |
| `TWITTER_OAUTH2_CLIENT_ID` | はい | Twitter OAuth 2.0 Client ID |
| `TWITTER_OAUTH2_CLIENT_SECRET` | いいえ | Twitter OAuth 2.0 Client Secret。public clientでは未指定可 |
